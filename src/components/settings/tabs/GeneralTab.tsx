import React from 'react';
import { FiSettings } from 'react-icons/fi';
import { useSettings } from '@/contexts/SettingsContext';

export const GeneralTab: React.FC = () => {
  const { settings, setSettings } = useSettings();

  return (
    <div className="space-y-6">
      <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-md mb-6">
        <h3 className="text-lg font-medium text-blue-800 dark:text-blue-300 flex items-center">
          <FiSettings className="mr-2" />
          General Settings
        </h3>
        <p className="text-sm text-blue-700 dark:text-blue-400 mt-1">
          Configure general application settings.
        </p>
      </div>
      
      <div className="grid grid-cols-1 gap-6">
        <div>
          <div className="flex items-center justify-between">
            <div>
              <label htmlFor="autoUpdateCustomNodes" className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                Auto-update Custom Nodes
              </label>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                Automatically update custom nodes when the application starts.
              </p>
            </div>
            <div className="ml-4">
              <label className="relative inline-flex items-center cursor-pointer">
                <input
                  type="checkbox"
                  id="autoUpdateCustomNodes"
                  className="sr-only peer"
                  checked={settings.autoUpdateCustomNodes || false}
                  onChange={(e) => setSettings({ ...settings, autoUpdateCustomNodes: e.target.checked })}
                />
                <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-blue-600"></div>
              </label>
            </div>
          </div>
        </div>
        
        <div>
          <div className="flex items-center justify-between">
            <div>
              <label htmlFor="startComfyUIOnBoot" className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                Start ComfyUI on Boot
              </label>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                Automatically start ComfyUI when the application starts.
              </p>
            </div>
            <div className="ml-4">
              <label className="relative inline-flex items-center cursor-pointer">
                <input
                  type="checkbox"
                  id="startComfyUIOnBoot"
                  className="sr-only peer"
                  checked={settings.startComfyUIOnBoot || false}
                  onChange={(e) => setSettings({ ...settings, startComfyUIOnBoot: e.target.checked })}
                />
                <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-blue-600"></div>
              </label>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};
